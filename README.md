...
调度器按**微内核粒度锁步轮转**（每个 MAC 后让出，每拍各 block 的 kr 同步
前进一步），动画上可见所有 block 同步并行计算、竞争共享 L2。切 j2 轴时
A 面板跨 block 共享 L2（B 独享）、切 i2 轴时 B 共享（A 独享）——共享维的
选择直接改变 L2 竞争模式。
...
**计算单元数**可单独设置（自动 / 1 / 2 / 4 / 8，类比 CUDA SM）。自动 =
跟随并行块数，每 block 独享一个单元、计算完全并行；设为具体值且小于
块数时，多个 block 轮转分派到同一单元（block b → 单元 b % nCores），
其计算在单元上**串行排队**——观察计算资源受限下加速比被压制的现象。

时间模型：`totalTime = max(最忙计算单元负载, 共享访存时间)`。计算主导
场景加速比 ≈ min(计算单元数, 块数)；访存主导场景受共享 L2/DRAM 带宽
压制。串行总时间（事件轨迹叠加）与并行总时间同时在统计面板给出，
加速比 = 串行 / 并行。
