...
- **时间模型**：带宽 + 峰值吞吐（`BW = {DRAM 16, L2 64, L1 256, Reg 512} GB/s`，
  `PEAK = 64 GFLOPS`，float64），延迟忽略；
...
| 内存层次 | DRAM/L2/L1/寄存器驻留块、搬运飞行动画、命中✓/未中✗/淘汰/超容量反馈、容量占用条 |
...
- 速度 0.25×–20×；「⚙ 参数」可改矩阵尺寸、tile 尺寸、缓存容量、随机种子；
- C 视图可切换「数值(部分和) / 访问热度」。
...
1. **带宽/延迟模型**：只计带宽不计延迟；串行回放（真实 BLIS 靠双缓冲/预取把
   搬运与计算重叠，故实际可达峰值 ~90%，本演示的「实际回放」点低于峰值墙，
   而「理论」点按重叠假设绘制——两者差距即流水线的价值）；
